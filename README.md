...
- **느낀 점**:
  컴포넌트로 분리하면서 props와 emit 흐름이 헷갈렸지만, 데이터 관리를 한곳에 모으고 자식은 화면 렌더링에만 집중시키니 코드가 훨씬 깔끔해졌다. 특히 slot을 활용해 공통 카드 디자인을 재사용하는 방식을 배울 수 있었다.
...
- **진행한 내용**:
  ...
  추가적으로 전국 기온 통계와 랭킹을 볼 수 있는 /stats 라우트와 잘못된 주소 접근 시 띄워주는 404 페이지를 구현하였다.
...
- **진행한 내용**:
  Pinia를 이용해 configStore를 생성하고 온도 단위 상태와 기호, 단위 변환 함수를 전역으로 관리하였다. 상단 네비게이션에 버튼을 배치하여 버튼 클릭 시 섭씨와 화씨가 바뀌도록 하였다.

  추가적으로 메인 날씨 카드뿐만 아니라 상세 페이지와 전국 통계 페이지의 모든 기온 수치도 전역 스토어의 단위에 맞춰 바뀌도록 하였다.
...
- **진행한 내용**:
  발급받은 API 키를 연동하여 도시의 실시간 기상 관측 데이터를 Promise.all로 병렬 호출하였다. 데이터 수신 중에는 isLoading으로 로딩 상태를 표시하고, try-catch로 통신 에러를 처리하였다.

  추가적으로 외부 조언 API(Advice Slip API)를 연동하여 날씨 맞춤 외출 팁과 글로벌 명언을 실시간으로 가져오는 기능을 추가하였다.

- **느낀 점**:
  실제 위성 날씨 데이터를 axios로 가져와 화면에 렌더링해보니 재미있었고 외부 서버와 비동기 통신을 할 때 로딩 상태 처리와 try-catch 예외 처리의 중요성을 알게 되었다.
...
- **진행한 내용**:
  ...
  추가적으로 상세 화면에 습도 수치를 el-progress 게이지바로 시각화하고, 실시간 한국 시계와 날씨 상태별 다이내믹 컬러 테마를 적용하였다.

- **느낀 점**:
  Element Plus 컴포넌트들을 활용하니 개발 속도가 훨씬 빨라지고 완성도 높은 대시보드를 만들 수 있었다.
